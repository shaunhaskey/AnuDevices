...
class PULSE_BLASTER(Device):
    '''Device to configure pulse blasters
    '''
    parts=[{'path':':HOSTIP','type':'text','value':'150.203.179.4:8052','options':('no_write_shot',)},]
    ...
    #This has to go to a MDSIP server, not a jServer!!! Make sure this MDSIP sever is running
    parts.append({'path':':FORCE_INIT','type':'action',
                  'valueExpr':"Action(Dispatch('150.203.179.70:8004','',50,None),Method(None,'INIT',head))",
                  'options':('no_write_shot',)})
    #This has to go to a MDSIP server, not a jServer!!! Make sure this MDSIP sever is running
    parts.append({'path':':FORCE_DAQ','type':'action',
                  'valueExpr':"Action(Dispatch('150.203.179.70:8004','',50,None),Method(None,'FORCE_TRIG',head))",
                  'options':('no_write_shot',)})

    parts.append({'path':':T0','type':'numeric'})
    parts.append({'path':':BOARD_NUMBER','type':'numeric'})

    parts.append({'path':':PROGRAM','type':'structure'})
    parts.append({'path':':CLOCK','type':'structure'})
    for i in range(1,5):
        parts.append({'path':':CLOCK:{}'.format(i),'type':'numeric'})
    ...
    for i in range(1,21):
        ...
        for j in list:
            if j=='NAME':
                parts.append({'path':':T%d:%s'%(i,j),'type':'text', 'value':''})#,'value':10})
            else:
                parts.append({'path':':T%d:%s'%(i,j),'type':'numeric'})#,'value':10})

    def init(self, arg):
        '''initialise the pulse blaster!!!
        SH:1May2013
        '''
        print 'Pulse blasters MDSplus initialisation - SH:1May2013'
        try:
            error = None
            print '####################################################'
            print 'Pulse blasters Init has started : MDSplus Device, SH 19Mar2013'
    ...
    def store(self, arg):
        ...
        '''
        print '####################################################'
        print 'Pulse blasters Store has started : MDSplus Device, SH 19Mar2013'
        try:
            error = None
